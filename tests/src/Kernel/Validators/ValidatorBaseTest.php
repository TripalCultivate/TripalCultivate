...
class ValidatorBaseTest extends ChadoTestKernelBase {
  ...
  protected function setUp(): void {
    ...
    $this->plugin_manager = \Drupal::service('plugin.manager.trpcultivate');
  }
  ...
  public function testSplitRowIntoColumns(string $expected_mime_type, string $expected_delimiter) {
    ...
    try {
      TripalCultivateValidatorBase::splitRowIntoColumns($str_line, 'text/uncertain');
    }
    catch (\Exception $e) {
      $exception_caught = TRUE;
      $exception_message = $e->getMessage();
    ...
    try {
      TripalCultivateValidatorBase::splitRowIntoColumns($str_line, $expected_mime_type);
    }
    catch (\Exception $e) {
      $exception_caught = TRUE;
      $exception_message = $e->getMessage();
  ...
  public function testFileDelimiterGetter(string $scenario, string $mime_type_input, bool $has_exception, string $exception_message, array|false $expected) {
    ...
    try {
      $delimiter = TripalCultivateValidatorBase::getFileDelimiters($mime_type_input);
    }
    catch (\Exception $e) {
      $exception_caught = TRUE;
